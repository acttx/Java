...
public class FundScreen {
	...
	private static void keep(Scanner kybd, Portfolio portfolio) {
		...
		if (TICKER.equals(keeping)) {
			...
			portfolio.removeIf(new Predicate<MutualFund>() {				
				public boolean test(MutualFund t) {
					return !(t.getAvgHoldingSize() >= floor && t.getAvgHoldingSize() <= ceiling); 
				}
			});
			...
			portfolio.removeIf(new Predicate<MutualFund>() {				
				public boolean test(MutualFund t) {
					return !(t.getValueMeasure() >= floor && t.getValueMeasure() <= ceiling); 
				}
			});
	...
	private static void exclude(Scanner kybd, Portfolio portfolio) {
		...
		if (TICKER.equals(removing)) {
			...
			String ticker = kybd.next().toUpperCase();
			//  TODO call portfolio.removeIf(...);
			portfolio.removeIf(t -> ticker.equals(t.getTicker()));
			...
			double ceiling = kybd.nextDouble();
			//  TODO call portfolio.removeIf(...);
			portfolio.removeIf((t -> t.getAvgHoldingSize() >= floor && t.getAvgHoldingSize() <= ceiling));
			...
			double ceiling = kybd.nextDouble();
			//  TODO call portfolio.removeIf(...);
			portfolio.removeIf((t -> t.getMinimumInvestment() >= floor && t.getMinimumInvestment() <= ceiling));
			...
			double ceiling = kybd.nextDouble();
			//  TODO call portfolio.removeIf(...);
			portfolio.removeIf((t -> t.getValueMeasure() >= floor && t.getValueMeasure() <= ceiling));
			...
			//  TODO call portfolio.removeIf(...);
			portfolio.removeIf(t -> t.getDomicile() == mkt);
		}
	}
}
